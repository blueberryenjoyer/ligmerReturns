...
an array argument value, where each element in the array is a string representing a "target" that JARVIS has found (i.e. medicine, food, etc.). HOWEVER,
sometimes JARVIS will detect a human-shaped lifeform, which could either be a zombie or a raider. In those cases, the targetsFoundWhileForaging array can
sometimes be given strings (ie. either a string of "zombie" or "raider"). We want to help JARVIS avoid those targets.

Your function should output the usual string to the terminal if JARVIS finds some good food or supplies while on his journey. That string, to remind you,
looks like this: "JARVIS has found some ______!" where the blank represents the type of food or supplies.

However, if JARVIS encounters a potential danger (either "zombie" or "raider"), we want our function to log  the following to the console: "JARVIS has found a _____!!
ENGAGING AVOIDANCE ACTION PLAN!!"
...
function raiderAvoidancePlan(targetsFoundWhileForaging) {
    ...
    for (let i = 0; i < targetsFoundWhileForaging.length; i++) {
        if (targetsFoundWhileForaging[i] === 'zombie' || targetsFoundWhileForaging[i] === 'raider') {
            console.log(`JARVIS has found a ${targetsFoundWhileForaging[i]}! run away!!`)
        }
        else {
...
console.log(raiderAvoidancePlan(['thing', 'another thing', 'zombie', 'zombie', 'corn syrup', 'raider', 'covid-19']))